...
def make_train_test_split(args, age_task_entries):
    if args.balanced_data:
        ...
        d_train, d_test = train_test_split(age_task_entries, test_size=args.test_ratio, random_state=args.seed,
                                           stratify=[entry['bb_age'] for entry in age_task_entries])
...
def calc_random_acc_score(args, model, test_dataloader):
    ...
    optimizer = None
    epoch = None
    val_loss, val_acc, val_old_acc, val_young_acc, avg_score = calc_leak_epoch_pass(epoch, test_dataloader, model, optimizer, False, print_every=500)

    return val_acc, val_loss, val_old_acc, val_young_acc, avg_score
...
def calc_leak(args, model, train_dataloader, test_dataloader):
    ...
    if args.calc_mw_acc:
        print('val, {0}, val loss: {1:.2f}, Male val acc: {2:.2f}'.format(epoch, val_loss*100, val_old_acc *100))
        print('val, {0}, val loss: {1:.2f}, Feale val acc: {2:.2f}'.format(epoch, val_loss*100, val_young_acc *100))
...
def calc_leak_epoch_pass(epoch, data_loader, model, optimizer, training, print_every):
    ...
    if args.store_topk_age_pred:
        all_old_pred_values, all_young_pred_values = [], []
        all_old_inputs, all_young_inputs = [], []

    total_score = 0 # for calculate scores

    cnt_data = 0
    for ind, (input_ids, attention_mask, token_type_ids, age_target, img_id) in tqdm(enumerate(data_loader), leave=False): # images are not provided
        ...
        if not training and args.calc_score:
            ...
            for i in range(pred_score_tensor.size(0)):
                old_score = F.softmax(predictions, dim=1).cpu().detach()[i,0]
                young_score = F.softmax(predictions, dim=1).cpu().detach()[i,1]
        ...
        if args.calc_mw_acc and not training:
            old_target_ind = [i for i, x in enumerate(age_target.cpu().numpy().tolist()) if x == 0]
            young_target_ind = [i for i, x in enumerate(age_target.cpu().numpy().tolist()) if x == 1]
    ...
    return t_loss / n_processed, acc, old_acc, young_acc, total_score / cnt_data
...
def main(args):
    ...
    if args.calc_ann_leak:
        ...
        if args.task == 'captioning':
            ...
            for caption_ind in range(5):
                trainANNCAPobject = BERT_ANN_leak_data(d_train, d_test, args, age_cap_entries, age_words, tokenizer,
                                                args.max_seq_length, split='train', caption_ind=caption_ind, data_dir=args.data_dir)
                testANNCAPobject = BERT_ANN_leak_data(d_train, d_test, args, age_cap_entries, age_words, tokenizer,
                                                args.max_seq_length, split='test', caption_ind=caption_ind, data_dir=args.data_dir)
                ...
                val_acc, val_loss, val_old_acc, val_young_acc, avg_score = calc_random_acc_score(args, model, test_dataloader)
                rand_acc_list.append(val_acc)
                rand_score_list.append(avg_score)
                # train and test
                val_acc, val_loss, val_old_acc, val_young_acc, avg_score = calc_leak(args, model, train_dataloader, test_dataloader)
    ...
    if args.calc_model_leak:
        ...
        if args.task == 'captioning':
            print('-- Task is Captioning --')
            d_train, d_test = make_train_test_split(args, age_cap_entries)
            trainMODELCAPobject = BERT_MODEL_leak_data(d_train, d_test, args, age_cap_entries, age_words, tokenizer,
                                                args.max_seq_length, split='train', pred_col=pred_col)
            testMODELCAPobject = BERT_MODEL_leak_data(d_train, d_test, args, age_cap_entries, age_words, tokenizer,
                                                args.max_seq_length, split='test', pred_col=pred_col)
            ...
            model = BERT_GenderClassifier(args, tokenizer)
            # calculate random predictions
            rand_val_acc, rand_val_loss, rand_val_old_acc, rand_val_young_acc, rand_avg_score = calc_random_acc_score(args, model, test_dataloader)
            # train and test
            val_acc, val_loss, val_old_acc, val_young_acc, avg_score = calc_leak(args, model, train_dataloader, test_dataloader)
