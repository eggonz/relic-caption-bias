...
def get_parser():
    ...
    parser.add_argument("--data_dir", default='bias_data', type=str)
...
def make_train_test_split(args, gender_task_mw_entries):
    if args.balanced_data:
        ...
        male_train_entries = [male_entries.pop(random.randrange(len(male_entries))) for _ in
                              range(each_train_sample_num)]
        female_train_entries = [female_entries.pop(random.randrange(len(female_entries))) for _ in
                                range(each_train_sample_num)]
        male_test_entries = [male_entries.pop(random.randrange(len(male_entries))) for _ in range(each_test_sample_num)]
        female_test_entries = [female_entries.pop(random.randrange(len(female_entries))) for _ in
...
def evaluate(model, iterator, criterion, batch_size, TEXT, args):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(iterator):
            ...
            if calc_mw_acc:
                probs = m(predictions).cpu()  # [batch_size]
                pred_genders = (probs >= 0.5000).int()
                male_target_ind = [i for i, x in enumerate(batch.label.to(torch.int32).cpu().numpy().tolist()) if
                                   x == 0]
                female_target_ind = [i for i, x in enumerate(batch.label.to(torch.int32).cpu().numpy().tolist()) if
                                     x == 1]
            ...
            if args.save_preds:
                ...
                for i, (imid, fs, pg) in enumerate(zip(batch.imid, probs, pred_genders)):
                    ...
                    all_pred_entries.append(
                        {'image_id': image_id, 'male_score': male_score, 'female_score': female_score,
                         'input_sent': sent})
    ...
    return epoch_loss / len(iterator), epoch_acc / len(
        iterator), total_score / cnt_data, male_acc, female_acc, male_score_avg, female_score_avg
...
def main(args):
    ...
    gender_obj_cap_mw_entries = pickle.load(
        open(f'{args.data_dir}/Human_Ann/gender_obj_cap_mw_entries.pkl', 'rb'))  # Human captions

    # Select captioning model
    if args.cap_model == 'nic':
        # selected_cap_gender_entries = pickle.load(open(f'{args.data_dir}/Show-Tell/gender_val_st10_cap_mw_entries.pkl', 'rb')) #Official
        # selected_cap_gender_entries = pickle.load(open(f'{args.data_dir}/Show-Tell/gender_val_st10_th2_cap_mw_entries.pkl', 'rb'))
        # selected_cap_gender_entries = pickle.load(open(f'{args.data_dir}/Show-Tell/gender_val_st10_th5_cap_mw_entries.pkl', 'rb'))
        selected_cap_gender_entries = pickle.load(
            open(f'{args.data_dir}/Show-Tell/gender_val_st10_th10_cap_mw_entries.pkl', 'rb'))
    elif args.cap_model == 'sat':
        if args.mask_bias_source == 'obj':
            print('### MASK OBJECT ###')
            selected_cap_gender_entries = pickle.load(
                open(f'{args.data_dir}/Show-Attend-Tell/gender_val_sat_masked_seg_obj_cap_mw_entries.pkl', 'rb'))
        elif args.mask_bias_source == 'person':
            print('### MASK PERSON ###')
            selected_cap_gender_entries = pickle.load(
                open(f'{args.data_dir}/Show-Attend-Tell/gender_val_sat_masked_seg_person_cap_mw_entries.pkl', 'rb'))
        elif args.mask_bias_source == 'both':
            print('### MASK BOTH ###')
            selected_cap_gender_entries = pickle.load(
                open(f'{args.data_dir}/Show-Attend-Tell/gender_val_sat_masked_seg_both_cap_mw_entries.pkl', 'rb'))
        else:
            selected_cap_gender_entries = pickle.load(
                open(f'{args.data_dir}/Show-Attend-Tell/gender_val_sat_cap_mw_entries.pkl', 'rb'))

    elif args.cap_model == 'fc':
        selected_cap_gender_entries = pickle.load(
            open(f'{args.data_dir}/Att2in_FC/gender_val_fc_cap_mw_entries.pkl', 'rb'))
    elif args.cap_model == 'att2in':
        selected_cap_gender_entries = pickle.load(
            open(f'{args.data_dir}/Att2in_FC/gender_val_att2in_cap_mw_entries.pkl', 'rb'))
    elif args.cap_model == 'updn':
        selected_cap_gender_entries = pickle.load(
            open(f'{args.data_dir}/UpDn/gender_val_updn_cap_mw_entries.pkl', 'rb'))
    elif args.cap_model == 'transformer':
        selected_cap_gender_entries = pickle.load(
            open(f'{args.data_dir}/Transformer/gender_val_transformer_cap_mw_entries.pkl', 'rb'))
    elif args.cap_model == 'oscar':
        if args.mask_bias_source == 'obj':
            print('### MASK OBJECT ###')
            selected_cap_gender_entries = pickle.load(
                open(f'{args.data_dir}/Oscar/gender_val_cider_oscar_masked_obj_cap_mw_entries.pkl', 'rb'))
        elif args.mask_bias_source == 'person':
            print('### MASK PERSON ###')
            selected_cap_gender_entries = pickle.load(
                open(f'{args.data_dir}/Oscar/gender_val_cider_oscar_masked_person_cap_mw_entries.pkl', 'rb'))
        elif args.mask_bias_source == 'both':
            print('### MASK BOTH ###')
            selected_cap_gender_entries = pickle.load(
                open(f'{args.data_dir}/Oscar/gender_val_cider_oscar_masked_both_cap_mw_entries.pkl', 'rb'))
        else:
            selected_cap_gender_entries = pickle.load(
                open(f'{args.data_dir}/Oscar/gender_val_cider_oscar_cap_mw_entries.pkl', 'rb'))
    elif args.cap_model == 'nic_equalizer':
        selected_cap_gender_entries = pickle.load(
            open(f'{args.data_dir}/Woman-Snowboard/gender_val_snowboard_cap_mw_entries.pkl', 'rb'))
    elif args.cap_model == 'nic_plus':
        selected_cap_gender_entries = pickle.load(
            open(f'{args.data_dir}/Woman-Snowboard/gender_val_baselineft_cap_mw_entries.pkl', 'rb'))
    ...
    masculine = ['man', 'men', 'male', 'father', 'gentleman', 'gentlemen', 'boy', 'boys', 'uncle', 'husband', 'actor',
                 ...
                 'emperor', 'emperors', 'dude', 'dudes', 'cowboy']
    ...
    if args.calc_ann_leak:
        ...
        if args.task == 'captioning':
            ...
            for cap_ind in range(5):
                ...
                train_proc = []
                valid_loss, valid_acc, avg_score, male_acc, female_acc, male_score_avg, female_score_avg = evaluate(
                ...
                valid_loss, valid_acc, avg_score, male_acc, female_acc, male_score, female_score = evaluate(model,
                                                                                                            test_iterator,
                                                                                                            criterion,
                                                                                                            args.batch_size,
                                                                                                            TEXT, args)
